...
Performs secret scanning (regex + Shannon entropy), basic AST checks,
...
def should_ignore(path: Path, ignore_paths: List[str]) -> bool:
    ...
    for ign in ignore_paths:
        if ign.endswith("/") and f"/{ign}" in f"/{path_str}/":
            return True
        if path_str.endswith(ign) or ign in path_str:
            return True
    return False
        ...
        for pat in patterns:
            for match in pat.finditer(line):
                ...
                findings.append({
                    ...
                    "file": str(path),
                    "line": lineno,
                    "fingerprint": secret[:6] + "…" + secret[-4:] if len(secret) > 12 else "***",
        ...
        for token in re.findall(r"[A-Za-z0-9/\+=_\-]{20,}", line):
            ...
            if ent >= entropy_threshold:
                findings.append({
                    ...
                    "recommendation": "Verify whether this is a secret. If yes, move to env/secret store.",
                })
    return findings
...
def ast_check_python(path: Path) -> List[Dict[str, Any]]:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Call):
            ...
            if name in {"get", "post", "put", "delete", "request", "head", "patch"}:
                has_timeout = any(
                    (isinstance(kw, ast.keyword) and kw.arg == "timeout")
                    for kw in node.keywords
                )
                if not has_timeout and isinstance(func, ast.Attribute):
                    findings.append({
                        "severity": "MEDIUM",
                        "rule": "missing-http-timeout",
                        "file": str(path),
                        "line": getattr(node, "lineno", 0),
                        "description": f"HTTP call '{name}' without explicit timeout",
...
def collect_files(root: Path, extensions: List[str], ignore_paths: List[str]) -> List[Path]:
    files = []
...
def main() -> int:
    ...
    extensions = cfg.get("python_extensions", [".py"]) + cfg.get("node_extensions", [".js", ".ts"])
    ...
    report = {
        "status": "FAILED" if any(s in counts for s in cfg["fail_on_severity"]) else "PASSED",
        "summary": {
    ...
    if args.json:
        ...
        if all_findings:
            print("\nFindings:")
            for f in all_findings:
                print(f"  [{f['severity']}] {f['file']}:{f.get('line', '?')} — {f['description']}")
